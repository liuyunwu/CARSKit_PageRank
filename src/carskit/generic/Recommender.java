...
import carskit.alg.cars.adaptation.dependent.sim.P_RANK;
import carskit.alg.cars.adaptation.dependent.sim.P_RANK_P;
    ...
    protected Map<Measure, Double> evalRankings() throws Exception {
        ...
        for (int u:uciList.keySet()) {
            ...
            for (int c : cis.keySet()) {
                ...
                if(algoName=="P_RANK_P") {
            	   	P_RANK_P.runPageRank(uu, c);
                } 
                ...
                for (Integer j : candItems) {
                  ...
                  //if(algoName=="P_RANK_P") {
                	   	//P_RANK_P.runPageRank(uu, c);
                    	//OLD--------------------------------------------------------------------------------------------------------------------------------------------
                    //itemScores.add(P_RANK_P.getRankItem(jj)); 
                    	// NEW---------------------------------------------------------------------------------------------------------------------------------------------
                    	//itemScores.get(jj).setValue(P_RANK_P.getScore(jj));
                    //}
                    if (!ratedItems.contains(j)) {
                        ...
                        if (!Double.isNaN(rank)) {
                            ...
                            if(rank>binThold)
                            	/******************************************************************************************************************************************
                            	 * MODIFICA IMPORTANTE
                            	 * BY DANIELE MONTERISI
                            	 * jj è l'item, rank è un punteggio assegnato all'item
                            	 * Ora scrivo una funzione che mi permette di prendere questi valori dal mio algoritmo
                            	 * che funziona così
                            	 * gli passo il nome dell'item e mi faccio restituire lo score 
                            	 */
                            ...
                            /**
                             * Se l'algoritmo è il PageRank allora gli item e i relativi score devono essere
                             * presi direttamente dalla classe PageRank
                             * 
                            ...
                            */
                            ...
                            /* Decommentare per tornare alla versione precedente
                             * if(algoName=="P_RANK") {
                            
                            	
                            	itemScores.add(P_RANK.getRankItem(jj, rank));
                            }
                             */
                        
                            /**
                             * ASSOLUTAMENTE SI; FUNZIONA RATE DAO per ottenere l'oggetto partendo dall'id
                             
                            	System.out.println(rateDao.getItemId(jj)+" "+rank);*/
                        }
                    } else {
                        numCands--;
                    }
                ...
                if(algoName=="P_RANK_P") {
                		...
                		for(int i=0; i<itemScores.size();i++) {
                			rankedItems.add(itemScores.get(i).getKey());
                			
                		     if (isResultsOut && count < numTopNRanks) {
                                 // restore back to the original item id
                                 sb.append("(").append(rateDao.getItemId(itemScores.get(i).getKey()));
                                 //System.out.println(item+ rateDao.getItemId(item));

                                 if (posItems.contains(itemScores.get(i).getKey()))
                                     sb.append("*"); // indicating correct recommendation

                                 sb.append(", ").append(itemScores.get(i).getValue().floatValue()).append(")");
                ...
                if (isResultsOut) {
                	...
                	//System.out.println(rateDao.getUserId(uu) + ", " + rateDao.getContextSituationFromInnerId(c) + ": " + sb.toString());
                    if (preds.size() >= 1000) {
                        FileIO.writeList(toFile, preds, true);
                        preds.clear();
                    }
